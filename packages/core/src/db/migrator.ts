...
/**
 * Apply any not-yet-applied .sql migrations from `migrationsDir`, in
 * filename order, tracked in _drizzle_migrations. Idempotent — safe to run
 * on every deploy. Used by both the CLI (migrate.ts via `sst shell`) and
 * the deploy-time migrator Lambda.
 */
export async function runMigrations(
  ...
  const alreadyApplied = new Set(
    (result.records ?? []).map((r) => r[0].stringValue!),
  );
  ...
  try {
    ...
    console.log(
      "No migrations directory found. Run 'npm run db:generate' first.",
    );
    return { applied: [] };
  }
